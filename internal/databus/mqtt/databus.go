...
func (databus Databus) handleMessage(_ mqtt.Client, msg mqtt.Message) {
	databus.messageHandler(msg.Topic(), msg.Payload())
}
...
func New(mqttServer string, topic string, msgHandler func(topic string, message []byte)) *Databus {
	databus := &Databus{}
	databus.topic = topic
	databus.messageHandler = msgHandler
